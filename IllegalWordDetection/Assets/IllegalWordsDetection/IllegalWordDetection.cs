...
{
    private static IllegalWordDetection _instance;

    public static IllegalWordDetection Instance
    {
        get
        {
            if (_instance == null)
                _instance = new IllegalWordDetection();
            return _instance;
        }
    }
    ...
    /// </summary>
    byte[] startCache = new byte[char.MaxValue];
    char[] dectectedBuffer = null;
    string SkipList = " \t\r\n~!@#$%^&*()_+-=【】、{}|;':\"，。、《》？αβγδεζηθικλμνξοπρστυφχψωΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ。，、；：？！…—·ˉ¨‘’“”々～‖∶＂＇｀｜〃〔〕〈〉《》「」『』．〖〗【】（）［］｛｝ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫ⒈⒉⒊⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩①②③④⑤⑥⑦⑧⑨⑩⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇≈≡≠＝≤≥＜＞≮≯∷±＋－×÷／∫∮∝∞∧∨∑∏∪∩∈∵∴⊥∥∠⌒⊙≌∽√§№☆★○●◎◇◆□℃‰€■△▲※→←↑↓〓¤°＃＆＠＼︿＿￣―♂♀┌┍┎┐┑┒┓─┄┈├┝┞┟┠┡┢┣│┆┊┬┭┮┯┰┱┲┳┼┽┾┿╀╁╂╃└┕┖┗┘┙┚┛━┅┉┤┥┦┧┨┩┪┫┃┇┋┴┵┶┷┸┹┺┻╋╊╉╈╇╆╅╄";
    BitArray SkipBitArray = new BitArray(char.MaxValue);
    /// <summary>
    /// 保有所有敏感词汇的最后一个词的记录，仅用来判断是否一个词是一个或者多个敏感词汇的“最后一个词”
    /// </summary>
    BitArray endCache = new BitArray(char.MaxValue);

    unsafe public void Init(string[] badwords)
    {
    ...
    unsafe string OriginalToLower(string text)
    {
    ...
    unsafe bool EnsuranceLower(string text)
    {
        fixed (char* newText = text)
        {
            char* itor = newText;
    ...
    /// <returns></returns>
    unsafe public string Filter(string text, string mask = "*")
    {
    ...
    /// <param name="text"></param>
    /// <returns></returns>
    unsafe public Dictionary<int, int> DetectIllegalWords(string text)
    {
